// Vendored from github.com/pimoroni/tufty2350 (modules/c/st7789/), the only
// driver Pimoroni publishes for the Tufty 2350's specific ST7789 variant.
// Local mods: stripped MicroPython glue (py/runtime.h and the cooperative-
// event hook inside the vsync wait loop). Driver logic itself unchanged.
#include "st7789.hpp"
  ...
  void ST7789::update() {
    ...
    if (use_vsync) {
      while (gpio_get(vsync) == 0) {
        // (upstream cooperatively pinged the MicroPython event loop here;
        // not relevant in our pico-sdk build)
      }
    }
